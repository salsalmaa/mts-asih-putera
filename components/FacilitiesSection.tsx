  ...
  useEffect(() => {
    const fetchFacilities = async () => {
      try {
        ...
        // Menyesuaikan struktur data dari response CMS Cration
        // (Biasanya data galeri berada di dalam data.Gallery atau langsung array)
        const galleryList = data?.Data?.Gallery || data?.Gallery || data || [];

        // Mapping format API CMS ke interface FacilityItem jika diperlukan
        const formattedData = galleryList.map((item: any) => {
  ...
  return (
    ...
    >
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-8 relative z-10">
        ...
        {/* Loading State */}
        {loading ? (
          <div className="text-center py-12 text-[#0a4b2f] font-medium">
            Memuat data fasilitas dari CMS...
          </div>
        ) : (
