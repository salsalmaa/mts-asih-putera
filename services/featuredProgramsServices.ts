...
export const featuredProgramsService = {
  getFeaturedPrograms: async (token?: string, cookieString?: string) => {
    console.log("📌 Calling Backend Go Fiber directly from Server...");
    ...
    try {
      const response = await axios.get(`${apiUrl}api/content?KanalType=K005`, { headers });
