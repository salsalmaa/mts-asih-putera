...
export const HeroClient: React.FC<HeroClientProps> = ({
  ...
  return (
    <section className="relative w-full overflow-hidden bg-[#0a4b2f]">
      {/* Background Media (Image or Video) with elegant overlay */}
      <div className="absolute inset-0 z-0">
        {loading ? (
          <div className="w-full h-full bg-[#0a4b2f] animate-pulse" />
          ...
          <video
            ...
            muted
            playsInline
            className="w-full h-full object-cover object-center scale-105 filter brightness-[0.88] contrast-[1.05] transition-all duration-1000 ease-in-out"
          />
        ) : (
          <img
            src={activeSlide.mediaUrl}
      ...
      <div className="relative z-10 max-w-7xl mx-auto px-4 sm:px-8 pt-8 sm:pt-12 pb-10 md:pb-16">
        <div className="max-w-2xl text-white">
          ...
          <div className="inline-flex items-center gap-2 mb-2">
            ...
            <span className="w-2 h-2 rotate-45 bg-[#d49b28]"></span>
            {slides.length > 1 && (
              <span className="ml-2 text-xs bg-black/30 border border-[#d49b28]/30 px-2 py-0.5 rounded-full text-[#f6d899]">
                {currentIndex + 1} / {slides.length}
              </span>
            )}
          </div>

          {/* Main Headline - Meaningfully adapted to support CMS dynamic slide title or fallback to core value statement */}
          <h1 className="font-sans text-3xl sm:text-4xl md:text-5xl lg:text-[50px] leading-[1.08] font-extrabold text-white mb-3.5 tracking-tight drop-shadow-sm transition-all duration-500">
